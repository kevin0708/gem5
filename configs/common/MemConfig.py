# author ssc
# date 2023/7/19
# Note：that version add some support for the DRAMsim3
...
def config_mem(options, system):
    ...
    opt_dramsim3_ini = getattr(
        options, "dramsim3_ini", None
    )  # add the support for DRAMsim3
    ...
    for r in system.mem_ranges:
        ...
        for i in range(nbr_mem_ctrls):
            if opt_mem_type and (not opt_nvm_type or range_iter % 2 != 0):
                ...
                if hasattr(m5.objects, "DRAMsim3") and issubclass(
                    intf, m5.objects.DRAMsim3
                ):
                    if opt_dramsim3_ini:
